...
(defun make-path-modification-form (form)
  (destructuring-bind (node-tag node-name path-direction direction) form
    ...
    (case path-direction
      (:next `(set-node-direction ,node-name ,direction))
      (:prev `(set-node-rev-direction ,node-name ,direction))
...
(defun make-node-insertion-form (form)
  (destructuring-bind (node-tag node-name direction-tag direction prev-tag prev-direction next-tag next-direction) form
    ...
    (let ((new-node-name (gensym "new-node")))
	 ...
	 (set-node-direction ,new-node-name ,next-direction)
	 (set-node-rev-direction ,new-node-name ,prev-direction)))))
...
(defun make-node-deletion-form (form)
  (destructuring-bind (node-tag node-name) form
    (declare (ignorable node-tag))
    `((set-node-direction ,node-name :none)
      (set-node-rev-direction ,node-name :none))))
...
(defmacro make-corner-swap-extension (&body body)
...
(defmacro make-corner-extension (&body body)
  (let* ((extension-direction (first (getf body :directions)))
	 (inv-extension-direction (invert-direction extension-direction))
	 (second-direction (second (getf body :directions)))
	 (inv-second-direction (invert-direction second-direction)))
    `(make-extension :name ,(getf body :name)
		     :category ,(getf body :name)
		     :conditions ((:is-free-node second-node :direction ,extension-direction)
				  (:is-free-node third-node :direction ,extension-direction))
		     :node-insertions ((:node second-node :direction ,extension-direction :prev ,inv-extension-direction :next ,second-direction)
				       (:node third-node :direction ,extension-direction :prev ,inv-second-direction :next ,inv-extension-direction))
		     :path-modifications ((:node second-node :next ,extension-direction)
					  (:node third-node :prev ,extension-direction)))))

 (defmacro make-corner-extensions (&body body)
  `(list (make-corner-swap-extension ,@body)
	 (make-corner-extension ,@body)))
