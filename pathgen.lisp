...
(defclass path-node ()
  ((row :initarg :row :initform 0 :accessor row)
   ...
   (direction :initarg :direction :initform :none :reader direction)
   (rev-direction :initarg :direction :initform :none :reader rev-direction)
   (grid :initarg :grid :accessor grid)
   (visited :accessor visited)
   (used-node-index :accessor used-node-index :initform nil)
...
(defclass grid ()
  ((width :initarg :width :initform 10 :accessor width)
   ...
   (used-nodes :initform (make-array 100 :adjustable t :fill-pointer 0) :accessor used-nodes)
   (unused-nodes :accessor unused-nodes :initform nil)
   (path-initial-node :accessor path-initial-node :initform nil)))
...
(defun insert-into-used-node-list (node)
  (assert (null (used-node-index node)))
  (if (null (unused-nodes (grid node)))
      (progn
	(setf (used-node-index node) (fill-pointer (used-nodes (grid node))))
	(vector-push-extend node (used-nodes (grid node))))
      (progn
	(let ((free-index (pop (unused-nodes (grid node)))))
	  (setf (used-node-index node) free-index)
	  (setf (aref (used-nodes (grid node)) free-index) node)
	  free-index))))
(defun remove-from-used-node-list (node)
  (assert (not (null (used-node-index node))))
  (push (used-node-index node) (unused-nodes (grid node)))
  (setf (aref (used-nodes (grid node)) (used-node-index node)) nil))
(defun set-node-direction (node direction)
  (if (null (used-node-index node))
      (unless (eq :none direction)
	(setf (used-node-index node) (insert-into-used-node-list node)))
      (when (and (eq :none direction) (eq :none (rev-direction node)))
	(setf (used-node-index node) (remove-from-used-node-list node))))
  (setf (slot-value node 'direction) direction))
(defun set-node-rev-direction (node direction)
    (if (null (used-node-index node))
      (unless (eq :none direction)
	(setf (used-node-index node) (insert-into-used-node-list node)))
      (when (and (eq :none direction) (eq :none (direction node)))
	(setf (used-node-index node) (remove-from-used-node-list node))))
    (setf (slot-value node 'rev-direction) direction))
(defun set-grid-node-direction (grid row column direction)
  (set-node-direction (grid-node grid row column) direction))
(defun set-grid-node-rev-direction (grid row column direction)
  (set-node-rev-direction (grid-node grid row column) direction))
  ...
  (for-all-rows (grid row)
    (for-all-columns (grid column)
	 ...
	 (destructuring-bind (erow ecolumn) (extend-in-direction row column direction)
	   (set-grid-node-rev-direction grid erow ecolumn (invert-direction direction))))))))
(defun clean-path (grid)
  (for-all-rows (grid row)
    (for-all-columns (grid column)
      (set-grid-node-direction grid row column :none))))
(defun initialize-initial-path (grid)
  (clean-path grid)
  (setf (path-initial-node grid) (grid-node grid 0 0))
  (for-all-rows (grid row)
    (set-grid-node-direction grid row 0 :down))
  (for-all-columns (grid column)
    (set-grid-node-direction grid (1- (height grid)) column :right))
  (set-grid-node-direction grid (1- (height grid)) (1- (width grid)) :none)
...
(defmethod print-object ((grid grid) stream)
  (print-unreadable-object (grid stream :type t)
	...
	(let ((node (grid-node grid row column)))
	 (format stream (if (available node) "[~a|~a]" "[~a.~a]")
...
(defun find-random-node-on-path (grid)
  (let ((nodes (used-nodes grid))
	node)
    (iter (until (and node (not (eq :none (direction node)))))
	  (setf node (aref nodes (random (fill-pointer nodes)))))
    node))
...
(defun path-length (grid)
  (unvisit-grid grid)
  (let ((node (path-initial-node grid)))
...
(defun copy-grid (grid)
  (let ((new-grid (make-instance 'grid :width (width grid) :height (height grid))))
    (for-all-nodes (grid node)
      (set-grid-node-direction new-grid (row node) (column node) (direction node))
      (set-grid-node-rev-direction new-grid (row node) (column node) (rev-direction node)))
    new-grid))
...
~a slength mul ~a slength mul rmoveto
" node-size-mm (get-ps-double-direction-defs) line-width-mm (column (path-initial-node grid)) (row (path-initial-node grid)))
...
(defun draw-spiral (grid width height start-row start-col)
  (when (and (> width 1)
   ...
   (iter (for col from start-col to (+ start-col width -2))
	 (set-grid-node-direction grid start-row col :right))
   (iter (for row from start-row to (+ start-row height -2))
	 (set-grid-node-direction grid row (+ start-col width -1) :down))
   (iter (for col from (+ start-col width -1) downto (1+ start-col))
	 (set-grid-node-direction grid (+ start-row height -1) col :left))
   (iter (for row from (+ start-row height -1) downto (1+ start-row))
	 (set-grid-node-direction grid row start-col :up))
   (set-grid-node-direction grid (1+ start-row) start-col :right)
...
(defun disable-disk (grid radius)
  (let ((row-center (floor (height grid) 2))
	(column-center (floor (width grid) 2)))
    (flet ((square (x) (* x x)))
      (for-all-nodes (grid node)
	(when (< (square radius) (+ (square (- (row node) row-center)) (square (- (column node) column-center))))
	  (setf (available node) nil))))))
(defun draw-center-row (grid)
  (let ((node (grid-node grid (floor (height grid) 2) 0)))
    (setf (path-initial-node grid) node)
    (iter (while (< (column (neighbouring-node node :right)) (1- (width grid))))
	  (set-node-direction node :right)
	  (set-node-rev-direction (neighbouring-node node :right) :left)
	  (setf node (neighbouring-node node :right)))))
